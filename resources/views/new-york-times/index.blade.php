@extends('layouts.app')
@section('title', 'New York Times API')
@section('content')
    <center>New York Times API News Feed</center>

    <div class="row">

        <div class="col-12 d-flex justify-content-end">
            <form id="updateArticlesForm" class="row" method="post" action="{{ route('newsApi.updateDatabaseRecords') }}"
                autocomplete="off">
                <div class="col-xs-12 col-md-12">
                    <div class="form-group">
                        <a href="{{ route('newYorkApi.fetchLatestArticles') }}"
                            onclick="return confirm('This action will trigger addition of 20 more records from the API. Are you sure to proceed?')"
                            class="btn btn-success">Fetch More Records</a>
                        <a href="{{ route('newsApi.deleteNeyYorkTimesArticles') }}"
                            onclick="return confirm('Are you sure to delete New York Times API articles?')"
                            id="clean-database" class="btn btn-danger">Clean Database</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadArticles(page, pageSize, query = '') {
                ...
                $.ajax({
                    url: '{{ route('newYorkApi.preloadArticles') }}',
                    type: 'GET',
                    data: {
                        page: page,
                        pageSize: pageSize,
